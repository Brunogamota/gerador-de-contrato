      ...
      {result.changes.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
          {result.changes.map((c) => (
            <div
              key={c.installmentIdx}
              className="flex flex-col gap-1 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.06]"
            >
...
export default function MDRAnalyzerPage() {
  ...
  // Base rows with custom spread overrides applied
  const rows = useMemo(() => {
  ...
  const metrics = useMemo(
    () => computeMetrics(rows, mix, monthlyVolume),
    [rows, mix, monthlyVolume],
  );
    ...
    <div className="-mx-8 -my-8 flex min-h-screen">
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-auto">
        ...
        <div className="flex-1 px-8 py-6 flex flex-col gap-5">
          ...
          {/* ── Optimization banner (shown after running optimization) ── */}
          {optimizationResult && (
            <OptimizationBanner
              result={optimizationResult}
