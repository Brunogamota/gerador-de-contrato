...
import type { StrategyProfile, BrandKey } from '@/lib/mdr-analyzer/types';
import {
  STRATEGY_CONFIG,
  SAMPLE_COST_DATA,
  SAMPLE_CLIENT,
  ...
  return (
    <div className="bg-[#111113] rounded-xl border border-white/[0.06] p-4 flex flex-col gap-2 min-w-0">
      ...
      <div className="flex items-end justify-between gap-2">
        <p className={cn('text-xs leading-snug', subPositive ? 'text-emerald-400' : 'text-white/40')}>
          {sub}
        </p>
...
function StrategyInsightsCard({ strategy, rows }: { strategy: StrategyProfile; rows: ReturnType<typeof computeRows> }) {
  const insights = useMemo(() => computeInsights(rows), [rows]);

  return (
    <div className="bg-[#18181B] rounded-2xl border border-white/[0.06] px-5 py-4 flex items-center gap-4 flex-wrap">
      {/* Icon + label */}
      ...
      <button className="text-xs font-semibold text-brand hover:text-brand/80 transition-colors whitespace-nowrap shrink-0">
        Ver detalhes →
      </button>
    </div>
  );
...
export default function MDRAnalyzerPage() {
  ...
  const rows = useMemo(() => computeRows(costData, strategy), [costData, strategy]);

  const metrics = useMemo(
    () => computeMetrics(rows, SAMPLE_CLIENT.transactionMix, SAMPLE_CLIENT.monthlyVolume),
    [rows],
  );

  const prevMetrics = useMemo(() => {
    const prevRows = computeRows(SAMPLE_COST_DATA['visa'], prevStrategy);
    return computeMetrics(prevRows, SAMPLE_CLIENT.transactionMix, SAMPLE_CLIENT.monthlyVolume);
  }, [prevStrategy]);
  ...
  return (
    // Escape the layout's px-8 py-8 to go edge-to-edge on the right
    <div className="-mx-8 -my-8 flex min-h-screen">
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-auto">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span
                ...
                style={{
                  color: stratCfg.color,
                  borderColor: stratCfg.color + '40',
                  backgroundColor: stratCfg.color + '15',
                }}
              >
                {stratCfg.label}
              </span>
            ...
            <div className="flex items-center gap-2">
              <button className="px-3 py-2 rounded-xl text-xs font-medium border border-white/10 text-white/60 hover:text-white hover:border-white/20 bg-white/5 transition-all">
                Exportar
              </button>
              <button className="px-3 py-2 rounded-xl text-xs font-medium border border-white/10 text-white/60 hover:text-white hover:border-white/20 bg-white/5 transition-all">
                Salvar análise
              ...
              <button
                ...
                onClick={() => {
                  // trigger recalculate with cycling strategy
                  const order: StrategyProfile[] = ['conservative', 'balanced', 'aggressive', 'max_margin'];
                  const next = order[(order.indexOf(strategy) + 1) % order.length];
                  setStrategy(next);
                }}
              >
        ...
        <div className="flex-1 px-8 py-6 flex flex-col gap-5">
          ...
          <StrategyInsightsCard strategy={strategy} rows={rows} />
      ...
      <AnalyzerSidebar
        ...
        onRecalculate={() => {
          // just re-render; rows are derived from strategy
        }}
      />
    </div>
  );
