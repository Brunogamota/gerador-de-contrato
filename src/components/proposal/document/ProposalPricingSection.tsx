...
export function ProposalPricingSection({ d, mdrMatrix, intlProposalPricing, setupIntl }: Props) {
  const tipo = (d as ProposalData & { tipoMercado?: string }).tipoMercado ?? 'brasil';
  const showBrasil = tipo !== 'intl';
  const showIntl   = tipo !== 'brasil' && !!(intlProposalPricing?.processingRate);
  const hasIntl = showIntl;
  ...
  const intlRows: [string, string][] = p ? [
    ...
    ...(p.year1Commitment   ? [['Ano 1 — Volume mínimo',  `$${p.year1Commitment} em taxas de processamento`] as [string,string]] : []),
    ...(p.year2Commitment   ? [['Ano 2 — Volume mínimo',  `$${p.year2Commitment} em taxas de processamento`] as [string,string]] : []),
    ...(p.connectPayoutRate || p.connectPayoutFlatFee ? [['Connect — Payout', `${p.connectPayoutRate ? p.connectPayoutRate + '%' : ''}${p.connectPayoutFlatFee ? ' + $' + p.connectPayoutFlatFee : ''} por payout`.trim()] as [string,string]] : []),
    ...(p.connectMonthlyFee    ? [['Connect — Mensalidade',    `$${p.connectMonthlyFee} / conta ativa / mês`] as [string,string]] : []),
  ...
  return (
    <div style={{ marginBottom: '32px' }}>
      ...
      {showBrasil && (
        <>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '24px' }}>
            ...
            <tbody>
              {fees.map(([feeLabel, valor, obs], i) => (
                <tr key={feeLabel} style={{ background: i % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                  <td style={S.feeLabel}>{feeLabel}</td>
      ...
      {hasIntl && intlRows.length > 0 && (
        <>
          <p style={{ ...S.sectionTitle, color: '#1d4ed8' }}>Precificação Internacional</p>
          ...
          <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '6px', padding: '8px 12px', marginBottom: '8px' }}>
            <p style={{ fontSize: '10px', color: '#1e40af', margin: 0 }}>
              <strong>Serviços incluídos:</strong> Processamento de pagamentos (Cartões, Wallets — Apple Pay/Google Pay, Payment Intel 3DS/AA/CAU/NT) · Connect · Antifraude
            </p>
          </div>
        </>
      )}
