...
function Field({
  label, value, onChange, prefix, suffix, placeholder = '0.00', readOnly,
}: {
  label: string;
  value: string;
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-xl border border-ink-200 p-4 flex flex-col gap-3">
      <p className="text-xs font-bold text-ink-800 underline underline-offset-2">{title}</p>
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <Section title="Processamento">
        <Field label="Taxa" value={v.processingRate} onChange={set('processingRate')} suffix="%" readOnly={readOnly} />
        <Field label="Fee fixo" value={v.processingFlatFee} onChange={set('processingFlatFee')} prefix="$" readOnly={readOnly} />
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-ink-600">Modelo</label>
          <input
            type="text"
            ...
            className={cn(
              'w-28 px-2 py-1.5 text-xs font-mono border rounded-lg focus:outline-none focus:ring-1 focus:ring-brand',
              readOnly ? 'border-ink-100 bg-ink-50 text-ink-500 cursor-default' : 'border-ink-200 bg-white text-ink-900',
            )}
          />
        </div>
      </Section>
      ...
      <Section title="Commitment">
        <Field label="Year 1 (in Stripe Fees)" value={v.year1Commitment} onChange={set('year1Commitment')} prefix="$" readOnly={readOnly} />
        <Field label="Year 2 (in Stripe Fees)" value={v.year2Commitment} onChange={set('year2Commitment')} prefix="$" readOnly={readOnly} />
      </Section>
      ...
      <Section title="Stripe Connect">
        ...
        <Field label="Mensalidade / conta" value={v.connectMonthlyFee} onChange={set('connectMonthlyFee')} prefix="$" readOnly={readOnly} />
        <Field label="Por conta ativada" value={v.connectActivationFee} onChange={set('connectActivationFee')} prefix="$" readOnly={readOnly} />
      </Section>

      {/* Radar */}
      <Section title="Radar">
        <Field label="Standard / transação" value={v.radarStandardFee} onChange={set('radarStandardFee')} prefix="$" readOnly={readOnly} />
        <Field label="RFFTs / transação" value={v.radarRfftFee} onChange={set('radarRfftFee')} prefix="$" readOnly={readOnly} />
      </Section>

      {/* Payment Intel */}
      <Section title="Payment Intel">
        ...
        <Field label="Card Account Updater" value={v.intelCardUpdaterFee} onChange={set('intelCardUpdaterFee')} prefix="$" readOnly={readOnly} />
        <Field label="Network token" value={v.intelNetworkTokenFee} onChange={set('intelNetworkTokenFee')} prefix="$" readOnly={readOnly} />
      </Section>

      {/* Outras Tarifas */}
      <Section title="Outras Tarifas">
